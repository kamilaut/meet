...
const getAccessToken = async () => {
  const tokenCheckResult = await tokenCheck();
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url = 'https://c3oc9mae86.execute-api.eu-central-1.amazonaws.com/dev/api/get-events/{access_token}' + '/' + token;
...
const getToken = async (code) => {
  try {
    ...
    const response = await fetch(
      'https://c3oc9mae86.execute-api.eu-central-1.amazonaws.com/dev/api/token/{code}' + '/' + encodeCode
    );
    if (!response.ok) {
